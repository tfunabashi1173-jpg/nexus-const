...
import { fetchProjects, fetchAllAddons, fetchPartners, fetchSales, fetchCosts, getDashboardSummary, getSystemSetting, computeAlerts } from '@/lib/db'
...
export default async function DashboardPage() {
  ...
  const [projects, addons, partners, sales, costs] = await Promise.all([
    ...
    fetchSales(),      // アラートダイアログ用（キャッシュ済み）
    fetchCosts(),      // アラートダイアログ用（キャッシュ済み）
  ])
  const alertItems = computeAlerts(projects, sales, costs)
  end()
